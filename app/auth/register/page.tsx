...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      showAlert("รหัสผ่านสั้นเกินไป", "รหัสผ่านต้องมีความยาวอย่างน้อย 6 ตัวอักษร", "error");
      setIsLoading(false);
      return;
    }

    if (password !== confirmPassword) {
      showAlert("ข้อมูลไม่ถูกต้อง", "รหัสผ่านไม่ตรงกัน กรุณาตรวจสอบอีกครั้ง", "error");
      setIsLoading(false);
      return;
    }
    ...
    try {
      ...
      if (data.user?.identities?.length === 0) {
        showAlert("สมัครสมาชิกไม่สำเร็จ", "อีเมลนี้ถูกใช้งานไปแล้ว กรุณาใช้อีเมลอื่น", "error");
        setIsLoading(false);
        return;
      }
      ...
      showAlert("เกิดข้อผิดพลาด", err.message || "เกิดข้อผิดพลาดในการสมัครสมาชิก", "error");
      setIsLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-background)] px-4 py-12 sm:px-6 lg:px-8">
      <div className="card max-w-md w-full p-8 space-y-8 animate-luxury">
        ...
        <form className="space-y-5" onSubmit={handleRegister}>
          <div className="space-y-4">
            <div className="space-y-1">
              <label className="text-xs font-black uppercase tracking-widest text-[var(--color-text-muted)]">ชื่อ - นามสกุล</label>
              <div className="relative">
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase tracking-widest text-[var(--color-text-muted)]">รหัสผ่าน (ขั้นต่ำ 6 ตัว)</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            <div className="space-y-1">
              <label className="text-xs font-black uppercase tracking-widest text-[var(--color-text-muted)]">ยืนยันรหัสผ่านอีกครั้ง</label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <Lock className="h-5 w-5 text-gray-400" />
                </div>
        ...
        <div className="text-center text-sm">
          <span className="text-[var(--color-text-muted)] font-medium">มีบัญชีผู้ใช้แล้ว? </span>
          <Link href="/auth/login" className="font-bold text-[var(--color-brand)] hover:text-[var(--color-brand-dark)] transition-colors">
            เข้าสู่ระบบเลย
          </Link>
        </div>
      </div>
